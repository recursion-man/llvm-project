...
template <typename OpType>
LogicalResult LowerToEmitCCallOpaque<OpType>::matchAndRewrite(OpType op, PatternRewriter &rewriter) const {
...
// Insert #include <math.h> at the beginning of the module
  ...
  builder.create<emitc::IncludeOp>(moduleOp.getLoc(), builder.getStringAttr("math.h"));
...
std::unique_ptr<OperationPass<mlir::ModuleOp>> mlir::createConvertMathToEmitCPass() {
